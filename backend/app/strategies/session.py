...
# PH desk windows (local time = UTC+8) — strategy follows the clock
ASIA_PH_START = 7
ASIA_PH_END = 20  # exclusive → PH 7:00AM–7:59PM → EMA_RSI_Scalp

SMC_PH_START = 20  # PH 8:00PM
SMC_PH_END = 2  # exclusive → through 1:59AM → Liquidity_Sweep_SMC

EARLY_EMA_PH_START = 2
EARLY_EMA_PH_END = 7  # exclusive → PH 2:00AM–6:59AM → EMA_RSI_Scalp
...
def classify_asia_desk(ts: datetime) -> SessionWindow:
    ...
    return SessionWindow(
        SessionTier.AVOID,
        "outside_asia_desk",
        "Outside desk hours — next window PH 7:00AM",
    )
