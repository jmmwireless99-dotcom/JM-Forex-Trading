//+------------------------------------------------------------------+
//| JM_GOLD_SMALL_CANDLE_FLOW_SCALPER_V4_2.mq5                      |
//| GOLD# micro scalper: M5 market flow + M1 small-candle breakout   |
//| V6.1-FINAL-H16-H17-BLOCK + live OK/MALI analyzer (display only)  |
//+------------------------------------------------------------------+
#property strict
#property version   "6.10"
#property copyright "JM Tech Solution"
#property description "GOLD# Small Candle Flow Scalper - M5 flow, M1 compression breakout, USD targets"
...
input long   InpMagic               = 26091061;
input string InpMasterBuildTag      = "V6.1-FINAL-H16-H17-BLOCK";
...
input double InpQuietTargetUsd      = 3.0;
input double InpNormalTargetUsd     = 5.0;
input double InpStrongTargetUsd     = 10.0;
...
{
   if(atr<=InpM1AtrQuiet) return InpQuietTargetUsd;
   if(atr>=InpM1AtrStrong) return InpStrongTargetUsd;
...
bool OpenTrade(int dir,double atr,string reason)
{
   ...
   Print("=== V6.0 MASTER CLEAN LOADED | ",InpMasterBuildTag," | Symbol=",g_symbol," | Lot=",DoubleToString(InpLots,2)," ===");
...
#include <JM_V61_LiveAnalyzer.mqh>
void OnTick()
{
   LiveAnalyzerTick();
